...
{
	// 변수 선언
	// -- static 으로 선언해서 필요할 때마다 만드는 것이 아니라 미리 하나 만든다.
	// -- 즉, Connection는 한 번만 만들고 필요 할 때 꺼내쓴다. (열쇠 예시)
	...
	public static Connection getConnection()
	{
		...
		if(dbConn == null)
		{
			try
			{
				...
				//-- 오라클 서버 실제 연결★
				//   위 (line 38 ~ 46)는...연결을 위한 환경을 설정하는 과정
				//   갖고있는 인자값(매개변수)은 오라클주소, 계정명, 패스워드
				
				
			}catch (Exception e)// (ClassNotFondException, SQLException)
			{
	...
	{
		// dbConn 변수(멤버 변수)는 
		// Database 가 연결관 상태일 경우 Connection 을 갖는다.
